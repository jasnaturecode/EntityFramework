...
namespace Microsoft.EntityFrameworkCore.Infrastructure
{
    public class SqlServerModelValidator : RelationalModelValidator
    {
        ...
        public override void Validate(IModel model)
        {
            ...
            EnsureNoNonKeySequenceValueGeneration(model);
        }
        ...
        protected virtual void EnsureNoNonKeySequenceValueGeneration([NotNull] IModel model)
        {
            foreach (var property in model.GetEntityTypes().SelectMany(t => t.GetDeclaredProperties())
                .Where(p => p.SqlServer().ValueGenerationStrategy == SqlServerValueGenerationStrategy.SequenceHiLo
                            && !p.IsKey()))
            {
                ShowError(SqlServerStrings.NonKeySequenceValueGeneration(property.Name, property.DeclaringEntityType.DisplayName()));
            }
        }
